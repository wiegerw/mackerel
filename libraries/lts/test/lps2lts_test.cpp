...
{
  std::clog << "Translating LPS to LTS with exploration strategy " << strategy << ", rewrite strategy "
            << rewrite_strategy << "." << std::endl;
                                        ...
                                        const std::string& priority_action = "")
{
  ...
  for (rewrite_strategy_vector::const_iterator rewr_strategy = rstrategies.begin();
       rewr_strategy != rstrategies.end(); ++rewr_strategy)
  {
    exploration_strategy_vector estrategies(exploration_strategies());
    for (auto expl_strategy: estrategies)
    {
      if (expl_strategy != es_breadth)
      {
        continue;
      }

      std::cerr << "AUT FORMAT\n";
      lts_aut_t result1 = translate_lps_to_lts<lts_aut_t>(lpsspec, expl_strategy, *rewr_strategy,
                                                                    priority_action);
      ...
      std::cerr << "LTS FORMAT\n";
      lts_lts_t result2 = translate_lps_to_lts<lts_lts_t>(lpsspec, expl_strategy, *rewr_strategy,
                                                                    priority_action);
      ...
      std::cerr << "FSM FORMAT\n";
      lts_fsm_t result3 = translate_lps_to_lts<lts_fsm_t>(lpsspec, expl_strategy, *rewr_strategy,
                                                                    priority_action);
