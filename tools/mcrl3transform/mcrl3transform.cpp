...
struct parse_process_specification_command: public process::detail::process_command
{
  parse_process_specification_command(const std::string& input_filename, const std::string& output_filename, const std::vector<std::string>& options)
    : process::detail::process_command("parse-process-specification", input_filename, output_filename, options)
  {}

  void execute() override
  {
    procspec = process::detail::parse_process_specification(input_filename);
    process::detail::save_process_specification(procspec, output_filename);
  }
};
...
class mcrl3transform_tool: public transform_tool<rewriter_tool<input_output_tool>>
{
  typedef transform_tool<rewriter_tool<input_output_tool>> super;

  public:
    mcrl3transform_tool()
      : super("mcrl3transform",
              "Wieger Wesselink",
              "applies a transformation to an mCRL3 process specification",
    ...
    void add_commands(const std::vector<std::string>& options) override
    {
      ...
      add_command(std::make_shared<parse_process_specification_command>(input_filename(), output_filename(), options));
    }
};

int main(int argc, char* argv[])
{
  return mcrl3transform_tool().execute(argc, argv);
}
